...
MEDIA_DIR = '/Users/starkat/Library/Application Support/Anki2/User 1/collection.media'  # Replace with your actual media directory path
...
def main():
    ...
    for audio in audio_files:
        ...
        if word:
            ...
            print(f"Filename '{audio}' does not match the expected pattern and will be skipped.")
    ...
    for word, audios in word_to_audios.items():
        ...
        for note in notes_info:
            ...
            updated_content = current_content + ' ' + audio_refs if current_content else audio_refs
            updated_content = updated_content.strip()
            update_note_field(note['noteId'], TARGET_FIELD, updated_content)
            print(f"Updated note ID {note['noteId']} with audio files: {', '.join(audios)}")
